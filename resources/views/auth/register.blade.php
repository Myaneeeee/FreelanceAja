                    ...
                    <form method="POST" action="{{ route('register.submit') }}">
                        ...
                        <div class="mb-4 p-3 bg-light rounded border">
                            ...
                            <div class="d-flex gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="role" id="role_freelancer"
                                        value="freelancer" {{ old('role', 'freelancer') == 'freelancer' ? 'checked' : '' }}>
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="role" id="role_client"
                                        value="client" {{ old('role') == 'client' ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name') }}" required
                                autofocus placeholder="{{ __('auth.placeholder_name') }}">
                            @error('name')
                                <span class="text-danger small">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label">{{ __('auth.contact_email') }}</label>
                                <input type="email" name="contact_email" class="form-control"
                                    value="{{ old('contact_email') }}"
                                    placeholder="{{ __('auth.contact_email_placeholder') }}" required>
                                <div class="form-text small">{{ __('auth.visible_to_contracts') }}</div>
                                @error('contact_email')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ __('auth.contact_phone') }}</label>
                                <input type="text" name="contact_phone" class="form-control"
                                    value="{{ old('contact_phone') }}"
                                    placeholder="{{ __('auth.contact_phone_placeholder') }}" required>
                                <div class="form-text small">{{ __('auth.visible_to_contracts') }}</div>
                                @error('contact_phone')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="d-grid gap-2 mt-4">
                            <button type="submit"
                                class="btn btn-primary btn-lg">{{ __('auth.create_account_btn') }}</button>
                        </div>
