...
@section('title', __('freelancer.dashboard'))

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-dark">{{ __('freelancer.welcome_back', ['name' => Auth::user()->name]) }}</h2>
            <p class="text-muted mb-0">{{ __('freelancer.dashboard_subtitle') }}</p>
        </div>
    ...
    @if (!$profile->bio || $profile->rate_per_hour == 0)
        <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center mb-4" role="alert">
            ...
            <div>
                <strong>{{ __('freelancer.complete_profile_title') }}</strong>
                {{ __('freelancer.complete_profile_desc') }}
                <a href="{{ route('freelancer.profile.edit') }}"
                    class="alert-link">{{ __('freelancer.edit_profile_link') }}</a>
            </div>
        </div>
    @endif
    ...
    <div class="row g-4 mb-5">
        <div class="col-md-6">
            <div class="card border-0 shadow-sm h-100 hover-shadow transition-all position-relative overflow-hidden">
                <div class="card-body p-4 d-flex align-items-center justify-content-between">
                    <div>
                        <h6 class="text-muted text-uppercase fw-semibold small mb-2">{{ __('freelancer.active_contracts') }}
                        </h6>
                    ...
                    <div class="bg-success bg-opacity-10 p-3 rounded-circle">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor"
                            class="text-success bi bi-briefcase" viewBox="0 0 16 16">
                            <path
        ...
        <div class="col-md-6">
            <div class="card border-0 shadow-sm h-100 hover-shadow transition-all position-relative overflow-hidden">
                <div class="card-body p-4 d-flex align-items-center justify-content-between">
                    <div>
                        <h6 class="text-muted text-uppercase fw-semibold small mb-2">{{ __('common.pending_proposals') }}
                        </h6>
                    ...
                    <div class="bg-secondary bg-opacity-10 p-3 rounded-circle">
                        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor"
                            class="text-secondary bi bi-send" viewBox="0 0 16 16">
                            <path
                        ...
                        @forelse($activeContracts as $contract)
                            <div class="list-group-item px-4 py-3 position-relative hover-bg-light">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    ...
                                    <span
                                        class="badge bg-success-subtle text-success rounded-pill px-3">{{ __('common.active') }}</span>
                                </div>
                                <p class="text-muted small mb-2">{{ __('freelancer.client') }}:
                                    {{ $contract->job->clientProfile->user->name }}
                                </p>
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="fw-bold small text-dark">Rp.
                                        {{ number_format($contract->final_price) }}</span>
                                    <small class="text-muted">{{ __('freelancer.started') }}
                                        {{ \Carbon\Carbon::parse($contract->start_date)->format('M d') }}</small>
                                </div>
                                {{-- Make row clickable --}}
                                <a href="{{ route('freelancer.jobs.show', $contract->job_id) }}"
                        ...
                        @empty
                            <div class="text-center py-5 px-4">
                                ...
                                <a href="{{ route('freelancer.jobs.index') }}"
                                    class="btn btn-outline-primary btn-sm">{{ __('freelancer.find_work') }}</a>
                            </div>
                        @endforelse
                    </div>
                </div>
        ...
        <div class="col-lg-5">
            <div class="card shadow-sm border-0 h-100">
                <div class="card-header bg-white border-bottom pt-4 px-4 pb-3">
                    <h5 class="fw-bold mb-0">{{ __('freelancer.recommended_for_you') }}</h5>
                </div>
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        @forelse($recommendedJobs as $job)
                            <div class="list-group-item px-4 py-3 position-relative hover-bg-light">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <small
                                        class="text-muted">{{ $job->type == 'hourly' ? __('freelancer.hourly_label') : __('freelancer.fixed_label') }}</small>
                        ...
                        @empty
                            <div class="text-center py-5 px-4">
                                <div class="mb-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48"
                                        fill="currentColor" class="text-muted opacity-25 bi bi-stars" viewBox="0 0 16 16">
                                        <path
                                ...
                                <p class="text-muted small mb-3">{{ __('freelancer.add_skills_prompt') }}</p>
                                <a href="{{ route('freelancer.skills.edit') }}"
                                    class="btn btn-sm btn-outline-primary">{{ __('freelancer.add_skills') }}</a>
                            </div>
                        @endforelse
                    </div>
                </div>
                <div class="card-footer bg-white border-top-0 text-center py-3">
                    <a href="{{ route('freelancer.jobs.index') }}"
                        class="text-decoration-none fw-bold small">{{ __('common.view_all_jobs') }}</a>
                </div>
            </div>
        </div>
    </div>
