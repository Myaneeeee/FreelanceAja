                    ...
                    <form action="{{ route('freelancer.profile.update') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="headline" class="form-control"
                                ...
                                placeholder="{{ __('freelancer.headline_placeholder') }}">
                            @error('headline')
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="number" name="rate_per_hour" class="form-control"
                                    value="{{ old('rate_per_hour', $profile->rate_per_hour) }}" step="0.01">
                                @error('rate_per_hour')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <h6 class="mt-4 mb-3 text-uppercase small text-muted fw-bold">
                            {{ __('freelancer.contact_details_shared') }}</h6>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label fw-bold">{{ __('freelancer.contact_email_label') }}</label>
                                <input type="email" name="contact_email" class="form-control"
                                    value="{{ old('contact_email', $profile->contact_email) }}"
                                    placeholder="{{ __('freelancer.contact_email_placeholder') }}">
                                @error('contact_email')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">{{ __('freelancer.contact_phone_label') }}</label>
                                <input type="text" name="contact_phone" class="form-control"
                                    value="{{ old('contact_phone', $profile->contact_phone) }}" placeholder="+62...">
                                @error('contact_phone')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <a href="{{ route('freelancer.profile.show') }}"
                                class="btn btn-light">{{ __('client.cancel') }}</a>
                            <button type="submit"
                                class="btn btn-primary px-4">{{ __('freelancer.save_changes') }}</button>
                        </div>
                    </form>
                </div>
            </div>
