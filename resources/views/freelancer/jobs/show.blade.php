                ...
                <div class="alert alert-success border-0 shadow-sm">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        @if ($existingProposal->attachment_path)
                            <a href="{{ asset('storage/' . $existingProposal->attachment_path) }}" target="_blank"
                                class="btn btn-sm btn-outline-success">
                                <i class="bi bi-file-earmark-pdf"></i> {{ __('client.view_pdf_proposal') }}
                            </a>
                        @endif
                    </div>
                </div>
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body p-4">
                        ...
                        <form action="{{ route('freelancer.jobs.proposals.submit', $job->id) }}" method="POST"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="bid_amount" class="form-control"
                                            value="{{ old('bid_amount', $job->budget) }}" step="0.01" required>
                                    </div>
                                    <small class="text-muted">{{ __('client.budget') }}: Rp.
                                        {{ number_format($job->budget) }}</small>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-bold">{{ __('freelancer.cover_letter') }}</label>
                                <textarea name="cover_letter" rows="6" class="form-control"
                                    placeholder="{{ __('freelancer.cover_letter_placeholder') }}" required>{{ old('cover_letter') }}</textarea>
                                @error('cover_letter')
                            ...
                            {{-- NEW FILE INPUT --}}
                            <div class="mb-4">
                                <label class="form-label fw-bold">{{ __('freelancer.attach_file_optional') }}</label>
                                <input type="file" name="attachment" class="form-control" accept="application/pdf">
                                <div class="form-text">{{ __('freelancer.upload_pdf_note') }}</div>
                                @error('attachment')
                                    <span class="text-danger small">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('freelancer.jobs.index') }}"
                                    class="btn btn-light me-2">{{ __('client.cancel') }}</a>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 mb-4">
                <div class="card-body">
                    ...
                    <p class="fw-bold mb-1">{{ $job->clientProfile->company_name ?? __('freelancer.individual_client') }}
                    </p>
                    <p class="text-muted small mb-3">{{ __('freelancer.member_since') }}
                        {{ $job->clientProfile->created_at->format('Y') }}</p>

                    @if ($job->clientProfile->website_url)
                        <a href="{{ $job->clientProfile->website_url }}" target="_blank"
