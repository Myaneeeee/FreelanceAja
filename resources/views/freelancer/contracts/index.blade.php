...
@section('title', __('freelancer.my_contracts_title'))

@section('content')
    <div class="row align-items-center mb-4">
        <div class="col-md-6">
            <h2 class="fw-bold mb-0">{{ __('freelancer.my_contracts_title') }}</h2>
            <p class="text-muted small mb-0">{{ __('freelancer.manage_your_active_projects') }}</p>
        </div>
        {{-- Search Bar --}}
            ...
            <form action="{{ route('freelancer.contracts.index') }}" method="GET" class="d-inline-block w-100 w-md-auto">
                <div class="input-group">
                    ...
                    <input type="text" name="q" class="form-control border-start-0"
                        placeholder="{{ __('freelancer.search_job_or_client_placeholder') }}" value="{{ request('q') }}">
                    ...
                    <thead class="bg-light">
                        <tr>
                            <th class="ps-4 py-3 text-secondary text-uppercase small" style="width: 40%">
                                {{ __('freelancer.job_client') }}</th>
                            <th class="py-3 text-secondary text-uppercase small">
                                {{ __('freelancer.start_date') ?? 'Start Date' }}</th>
                            <th class="py-3 text-secondary text-uppercase small">{{ __('freelancer.amount') }}</th>
                            ...
                            <tr>
                                <td class="ps-4 py-3">
                                    <div class="d-flex align-items-center">
                                        <div class="avatar bg-success bg-opacity-10 text-success rounded-circle text-center me-3 d-flex align-items-center justify-content-center"
                                            style="width: 40px; height: 40px;">
                                            <span
                                                class="fw-bold">{{ substr($contract->job->clientProfile->user->name, 0, 1) }}</span>
                                        </div>
                                        <div>
                                            <div class="fw-bold text-dark">{{ $contract->job->title }}</div>
                                            <small
                                                class="text-muted">{{ $contract->job->clientProfile->company_name ?? $contract->job->clientProfile->user->name }}</small>
                                        </div>
                                    </div>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    ...
                                    <h5 class="fw-bold">{{ __('freelancer.no_contracts_found') }}</h5>
                                    <p class="text-muted small">
                                        @if (request('status') == 'active')
                                            {{ __('freelancer.no_active_contracts_message') }}
                                        @else
                                            {{ __('freelancer.no_contract_history_message') }}
                                        @endif
                                    </p>
                                </td>
                            </tr>
