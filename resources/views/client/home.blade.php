...
<div class="row g-4 mb-5">
        ...
        <div class="card border-0 shadow-sm h-100 hover-shadow transition-all position-relative overflow-hidden">
            <div class="card-body p-4 d-flex align-items-center justify-content-between">
                ...
                <div class="bg-primary bg-opacity-10 p-3 rounded-circle">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="text-primary bi bi-briefcase" viewBox="0 0 16 16">
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm h-100 hover-shadow transition-all position-relative overflow-hidden">
            <div class="card-body p-4 d-flex align-items-center justify-content-between">
                <div>
                    <h6 class="text-muted text-uppercase fw-semibold small mb-2">{{ __('client.active_contracts') }}</h6>
                ...
                <div class="bg-success bg-opacity-10 p-3 rounded-circle">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="text-success bi bi-people" viewBox="0 0 16 16">
        ...
        <div class="card shadow-sm border-0 h-100">
            <div class="card-header bg-white border-bottom pt-4 px-4 pb-3 d-flex justify-content-between align-items-center">
                ...
                @if($activeContracts->count() > 0)
                    <a href="{{ route('client.contracts.index') }}" class="text-decoration-none small fw-bold">View All</a>
                @endif
            </div>
                        ...
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                ...
                                <p class="text-muted small mb-1">
                                    {{ __('client.freelancer') }}: <span class="text-dark">{{ $contract->freelancerProfile->user->name }}</span>
                                </p>
                            </div>
                            <span class="badge bg-success rounded-pill px-3">{{ $contract->status }}</span>
                        </div>
                        <div class="mt-2 d-flex justify-content-between align-items-center">
                            <span class="fw-bold text-secondary small">Rp. {{ number_format($contract->final_price) }}</span>
                            <small class="text-muted">Started: {{ $contract->start_date }}</small>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-5">
                        <div class="mb-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" class="text-muted opacity-25 bi bi-clipboard-x" viewBox="0 0 16 16">
                        ...
                        <p class="small text-muted">Accept proposals to start contracts.</p>
                    </div>
                @endforelse
                </div>
            </div>
        ...
        <div class="card shadow-sm border-0 h-100">
            ...
            <div class="card-body p-0">
                    ...
                    <div class="list-group-item px-4 py-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h6 class="fw-bold mb-0 text-truncate text-primary" style="max-width: 200px;">
                                <a href="{{ route('client.jobs.proposals', $proposal->job_id) }}" class="text-decoration-none">
                                    {{ $proposal->job->title }}
                                </a>
                            </h6>
                            <small class="text-muted">{{ $proposal->created_at->diffForHumans(null, true, true) }}</small>
                        </div>
                        <div class="d-flex align-items-center mb-2">
                            <div class="avatar bg-light rounded-circle text-center me-2" style="width:24px; height:24px; line-height:24px;">
                                <small class="fw-bold text-muted">{{ substr($proposal->freelancerProfile->user->name, 0, 1) }}</small>
                            </div>
                            <p class="small text-muted mb-0">{{ $proposal->freelancerProfile->user->name }}</p>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mt-2">
                            <span class="badge bg-light text-dark border">Bid: Rp. {{ number_format($proposal->bid_amount) }}</span>
                            <a href="{{ route('client.jobs.proposals', $proposal->job_id) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">
                                Review
                            </a>
                        </div>
                    </div>
                @empty
                    <div class="text-center py-5">
                         <div class="mb-3">
                            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" class="text-muted opacity-25 bi bi-inbox" viewBox="0 0 16 16">
                        ...
                        <small class="text-muted">Proposals will appear here when freelancers apply.</small>
                    </div>
                @endforelse
                </div>
            </div>
