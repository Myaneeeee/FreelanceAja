...
@section('content')
    <div class="row align-items-center mb-4">
        <div class="col-md-6">
            ...
            <p class="text-muted small mb-0">{{ __('client.manage_jobs_desc') }}</p>
        </div>
    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body p-3">
            <form action="{{ route('client.jobs.index') }}" method="GET" class="row g-2">
                <div class="col-md-8">
                    <div class="input-group">
                        ...
                        <input type="text" name="q" class="form-control border-start-0 ps-0"
                            placeholder="{{ __('client.search_by_job_title') }}" value="{{ request('q') }}">
                    </div>
                </div>
                <div class="col-md-3">
                    <select name="status" class="form-select">
                        <option value="">{{ __('freelancer.all_statuses') ?? 'All Statuses' }}</option>
                ...
                <div class="col-md-1">
                    <button type="submit"
                        class="btn btn-light w-100 border">{{ __('client.filter') ?? __('freelancer.filter') }}</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="ps-4 py-3 text-secondary text-uppercase small" style="width: 45%;">
                                {{ __('client.job_title') }}</th>
                            <th class="py-3 text-secondary text-uppercase small">{{ __('client.budget') }}</th>
                            <th class="py-3 text-secondary text-uppercase small">{{ __('client.proposals') }}</th>
                            <th class="py-3 text-secondary text-uppercase small">{{ __('client.status') }}</th>
                            <th class="py-3 text-end pe-4 text-secondary text-uppercase small">{{ __('client.action') }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="position-relative">
                                <td class="ps-4 py-3">
                                    <div class="d-flex flex-column">
                                        <a href="{{ route('client.jobs.proposals', $job->id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-5">
                                    ...
                                    <p class="text-muted mb-3">{{ __('client.create_job_prompt_desc') }}</p>
